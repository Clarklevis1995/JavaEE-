...
//        System.out.println(nightRush);
        Time minInNight = inNight(new Time(12, 20), new Time(2, 50));
    ...
    public static void invoiceCalculator() {
        ...
        nightFee = (int) (inNight(startTime, endTime).toMinute() * (totalMileage - 1) * STANDARD_PRICE * 0.2);
    ...
    public static Time inNight(Time start, Time end) {
        ...
        //起止时间在夜间范围内
        if ((start.toMinute() >= nightStart && end.toMinute() <= nightEnd) || (start.toMinute() >= 0 && start.toMinute() < nightEnd && end.toMinute() <= nightEnd) ||
